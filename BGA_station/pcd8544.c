...
void LcdInit ( void )
{
    ...
    LCD_DDR |= _BV( LCD_RST_PIN ) | _BV( LCD_DC_PIN ) | _BV( LCD_CE_PIN ) | _BV( SPI_MOSI_PIN ) | _BV( SPI_CLK_PIN ) | _BV(MAX_0_CS_PIN) |_BV( LCD_RST_PIN );// | _BV(MAX_1_CS_PIN)| _BV(PB0);
...
byte LcdChr ( LcdFontSize size, byte ch )
{
    ...
    if ( size == FONT_1X )
    {
        for ( ipp = 0; ipp < 5; ipp++ )
        {
            ...
            LcdCache[LcdCacheIdx++] = pgm_read_byte(&( FontLookup[ ch - 32 ][ ipp ] ) ) << 1;
        }
    }
    else if ( size == FONT_2X )
    {
        ...
        for ( ipp = 0; ipp < 5; ipp++ )
        {
            ...
            c = pgm_read_byte(&(FontLookup[ch - 32][ipp])) << 1;
            /* Enlarge image */
            /* First part */
            b1 =  (c & 0x01) * 3;
            b1 |= (c & 0x02) * 6;
...
void LcdUpdate ( void )
{
    ...
    for ( ixx = LoWaterMark; ixx <= HiWaterMark; ixx++ )
    {
        LcdSend( LcdCache[ ixx ], LCD_DATA );
    }
